...
@pytest.mark.parametrize(("inputs", "expected"), [
    ...
    ((np.array([3, 3, 3], dtype=np.float64), # x
      np.array([1, 1, 1], dtype=np.float64), # y
      {'mae': lambda x, y: float(tf.keras.losses.MAE(x.flatten(), y.flatten()).numpy())}, #custom_similarity_metrics
     ),
    ...
    (([np.array([3, 3, 3], dtype=np.float64), np.array([3, 3, 3], dtype=np.float64)], # x
      [np.array([1, 1, 1], dtype=np.float64), np.array([1, 1, 1], dtype=np.float64)], # y
      {'mae': lambda x, y: float(tf.keras.losses.MAE(x.flatten(), y.flatten()).numpy())}, #custom_similarity_metrics
     ),
    ...
    (({"x1":np.array([3, 3, 3], dtype=np.float64), "x2":np.array([3, 3, 3], dtype=np.float64)}, # x
      {"y1":np.array([1, 1, 1], dtype=np.float64), "y2":np.array([1, 1, 1], dtype=np.float64)}, # y
      {'mae': lambda x, y: float(tf.keras.losses.MAE(x.flatten(), y.flatten()).numpy())}, #custom_similarity_metrics
     ),
     {'mse': 0.0, 'cs': 0.0, 'sqnr': 0.0, 'mae': 0.0} # similarities
    ),
